...
async function chunkInsert<T extends any>(table: any, items: T[], chunkSize = 50) {
...
async function run() {
  console.log("Starting Supabase dynamic seeding sequence...");
  try {
    // 1. Seed users
    ...
    await chunkInsert(users, usersToInsert, 50);
    ...
    await chunkInsert(teachers, teachersToInsert, 50);
    ...
    await chunkInsert(grades, gradesToInsert, 50);
    ...
    console.log("Supabase seeding sequence completed successfully!");
    process.exit(0);
  } catch (err) {
